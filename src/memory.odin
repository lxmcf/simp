...
// We use Token_Keyword directly to avoid casting mismatches
Block_Ref :: struct {
    type:   Token_Keyword,
    index:  int,
    breaks: [dynamic]int,
...
_compute_tables :: proc(tokens: []Token, allocator := context.allocator) -> []int {
    ...
    for token_index := 0; token_index < len(tokens); token_index += 1 {
        ...
        case .While, .For, .Foreach, .Function, .If, .Array, .Object:
        ...
        case .None:
            if token.type == .RBrace {
                if len(stack) > 0 {
                    ...
                    has_else := false
                    else_idx := -1
                    for i := token_index + 1; i < len(tokens); i += 1 {
                        if tokens[i].type == .Newline {

                            continue
                        }
                        if tokens[i].keyword == .Else {
                            has_else = true
                            else_idx = i
                        }
                        break
                    ...
                    if block_ref.type == .If && has_else {
                        jump_table[block_ref.index] = else_idx
                        append(&stack, Block_Ref{type = .Else, index = token_index})
                        append(&stack, Block_Ref{type = .None, index = else_idx})
